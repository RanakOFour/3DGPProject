...
bool MeshCollider::IsColliding(Transform* _transform, Collider* _other, Transform* _otherTransform)
{
    ...
    if(_other->m_type == ColliderType::Box)
    {
        ...
        glm::vec3 L_othHalfSize = glm::vec3(L_boxOther->GetSize().Width() / 2.0f, L_boxOther->GetSize().Height() / 2.0f, L_boxOther->GetSize().Depth() / 2.0f);
        ...
        for(int i = 0; i < m_Triangles.size(); ++i)
		{
			...
			L_currentFace.a.position = m_Triangles[i].a.position - L_pos;
			L_currentFace.b.position = m_Triangles[i].b.position - L_pos;
			L_currentFace.c.position = m_Triangles[i].c.position - L_pos;
			...
			for(int j = 0; j < 3; ++j)
			{
				for(int k = 0; k < 3; ++k)
				{
					...
					float L_boxRadius = L_othHalfSize.x * L_currentAxis.x +
										L_othHalfSize.y * L_currentAxis.y +
										L_othHalfSize.z * L_currentAxis.z;
