...
Game::Game()
{
    std::shared_ptr<Game> L_this = std::shared_ptr<Game>(this);
...
void Game::LoadGame()
{
    ...
    const char* L_playerTexPath = "./resources/curuthers/Whiskers_defuse.png";
    // Create main scene
...
void Game::Play()
{
    ...
    do
    {
        ...
        const Uint8* L_sdlKeys = m_Window->Update();

        // Update scene with inputs from window

        m_Scene->Update(L_deltaTime, L_sdlKeys);
        // Wawa
